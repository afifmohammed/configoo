If you wish to access values from the <appconfig> section of your config or retreive a connection string 
	...
	- recommended usage is to instantiate it once and re-use.
	...
	- AppConfig provides ability to provide default values for keys on request.
	  If the key doesn't exist it will be added to an in memory dictionary 
	  with the default value when one is provided.
...
If you wish to provide overrides on how values are added to an in memory dictionary and how they are retreived
	...
	- Override the Get<TValue>(string key, TValue @default) method 
	  to provide custom behaviour for retreival and adding defaults
	- Override the IsNotNull<TValue>(TValue value) method to provide 
	  custom behaviour on how to evaluate if provided value is not null.
	- Is helpful when you
		- wish to talk to another set of configuration values besides the AppConfig
		- want the same unified interface that the AppConfig class provides.
		- are happy to store missing keys with default values (when provided) 
